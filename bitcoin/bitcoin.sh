#!/bin/sh
# ^ Shebang allows this script to be called from command line via './bitcoin.sh'

# Sets start time for this script to be used in timing
start=`date +%s`

# Sets the current directory as the 'root' of the bitcoin directory
BCT_PATH=$(pwd)

# Check if directory 'bitcoin' exists, if not, clone it from the repo
# Otherwise, pull from the repo and see if there have been changes
# since this script was run last, if not, exit
if [ ! -d "${BCT_PATH}/bitcoin" ]; then
...
# Check to see if lizard is installed, if so, run it on the repo and output the results to 'bitcoinlizarduncomp.txt'
# otherwise, print a message indicating lizard needs to be installed, then exit
if which lizard >/dev/null; then
	echo "Running lizard"
	lizard -l cpp > ../bitcoinlizarduncomp.txt
else
...
../../lizardtrim.py ../bitcoinlizarduncomp.txt
...
if which cppcheck >/dev/null; then
	...
	cppcheck -j 750 -q --force --enable=warning src 2>../bitcoincppcheck.txt
else
...
# Run lizard on the compiled code and output the results to the file 'bitcoinlizard.txt'
lizard -l cpp > ../bitcoinlizard.txt
...
echo "Trimming lizard results"
../../lizardtrim.py ../bitcoinlizard.txt
...
# The output file option names the resulting .info file whatever we wish to call it (Here, bitcoin_test)
# The redirect at the end simply directs the output of lcov to null as it spits out a lot of info we don't need
# while it is running
lcov --directory . --capture --output-file bitcoin_test.info > /dev/null 2>&1
...
genhtml -o ../../../../lcov bitcoin_test.info > /dev/null 2>&1
...
# in the file 'bitcointime.txt'
echo $((end - start)) > ../../../../bitcointime.txt
